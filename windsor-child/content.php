...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'post_item_single post_type_'.esc_attr(get_post_type()) 
	...
	if (windsor_is_on(windsor_get_theme_option('seo_snippets'))) {
		?>
		<div class="structured_data_snippets">
			...
			<div itemprop="publisher" itemscope itemtype="https://schema.org/Organization">
				<div itemprop="logo" itemscope itemtype="https://schema.org/ImageObject">
				    <?php 
					$windsor_logo_image = windsor_get_retina_multiplier(2) > 1 
										? windsor_get_theme_option( 'logo_retina' )
										: windsor_get_theme_option( 'logo' );
					if (!empty($windsor_logo_image)) {
						$windsor_attr = windsor_getimagesize($windsor_logo_image);
					    ?>
						<img itemprop="url" src="<?php echo esc_url($windsor_logo_image); ?>">
	...
	// Title and post meta
	if ( !get_query_var('windsor_title_showed', false) && !in_array(get_post_format(), array('link', 'aside', 'status', 'quote')) ) {
		?>
		<div class="post_header entry-header">
			<?php
			// Post title
			the_title( '<h3 class="post_title entry-title"'.(windsor_is_on(windsor_get_theme_option('seo_snippets')) ? ' itemprop="headline"' : '').'>', '</h3>' );
			// Post meta
